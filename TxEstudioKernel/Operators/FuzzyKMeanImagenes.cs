...
{
    [Algorithm("FuzzyKMeans Method", "No Supervised image segmentation using the KMean Algorithm")]
    [Abbreviation("km", "NumeroClases", "MaxIteraciones", "NumeroMuestras", "NumeroPruebas", "B")]
    public class FuzzyKMeanImagenes : TxSegmentationAlgorithm
    {
        double error;
        ...
        public override TxSegmentationResult Segment(params TxMatrix[] descriptors)
        {
            ...
            FuzzyKMeans kmean = new FuzzyKMeans(muestra, numeroClases, maxIteraciones, b);
            kmean.HallaPatrones();


            //Buscar el kmean que da mejores resultados
            FuzzyKMeans kmean1;
            error = kmean.Error();
            double error1;
            for (int i = 0; i < numeroPruebas; i++)
            {
                kmean1 = new FuzzyKMeans(muestra, NumeroClases, maxIteraciones, b);
