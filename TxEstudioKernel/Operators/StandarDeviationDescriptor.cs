...
namespace TxEstudioKernel.Operators
{
    ...
    [Algorithm("Standard Deviation", "Calculates the Standard Deviation feature of a grey levels in a Window of size Nx x My.")]
    [Abbreviation("fo_stdv", "nRows", "nCols")]
    public class StandarDeviationDescriptor :TextureDescriptor
    {
        ...
        public override TxMatrix GetDescription(TxImage image)
        {
            ...
            for (int j = nCols / 2; j < image.Height - (nCols / 2); j++)
            {
                for (int i = nrows / 2; i < image.Width - (nrows / 2); i++)
                {
                    ...
                    resultado[j,i] = (float)System.Math.Sqrt((double)resultadoParcial);
                    resultadoParcial = 0;
                }
            }
